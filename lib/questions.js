...
async function passwordToUpdate() {
  const { updatePasswordName } = await inquirer.prompt([
    {
      type: "input",
      name: "updatePasswordName",
      message: "Which password you want to update?",
    },
  ]);
  const { updateNewPasswordValue } = await inquirer.prompt([
    {
      type: "input",
      name: "updateNewPasswordValue",
      message: "Type your new password Value",
    },
  ]);
  return [updatePasswordName, updateNewPasswordValue];
}
...
exports.passwordToUpdate = passwordToUpdate;
